...
def test_every_endpoint_answers() -> None:
    for url in ("/api/health", "/api/status", f"/api/season/{SEASON}",
...
def test_every_market_can_be_resolved() -> None:
    ...
    from goalmodel.models.baseline import all_markets
...
def test_no_model_is_loaded() -> None:
    """
    Importing the API must not pull in LightGBM.

    IN A SUBPROCESS, AND THAT IS THE POINT. Read on the shared interpreter,
    `sys.modules` says what the whole test session has imported, not what the
    API imports: any earlier test that fits a model makes this one pass for
    the wrong reason, and the day someone moves `import lightgbm` to module
    level it would keep passing. A fresh interpreter answers the question that
    was actually asked.
    """
    codice = (
        "import sys; import backend.api; "
        "pesanti = [m for m in ('lightgbm', 'sklearn', 'statsmodels') if m in sys.modules]; "
        "print(','.join(pesanti)); "
        "sys.exit(1 if pesanti else 0)"
    )
    esito = subprocess.run([sys.executable, "-c", codice],
                           capture_output=True, text=True, check=False)
    assert esito.returncode == 0, (
        f"importing the API pulled in {esito.stdout.strip()}: "
        "keep heavy imports inside functions"
    )
