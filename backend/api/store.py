...
def _opt_int(value) -> int | None:
    ...
    return None if out is None else round(out)
...
def _read_predictions(src: Sources) -> pd.DataFrame:
    ...
    """
    from goalmodel.prediction.backtest_log import load_log
...
def _read_kickoffs(src: Sources) -> dict:
    ...
    if not src.schedule.exists():
        ...
        return {}
    try:
        from goalmodel.prediction.rounds import calendario
...
def _round_states(fingerprint: tuple, season: str) -> pd.DataFrame:
    ...
    """
    try:
        from goalmodel.prediction.rounds import stato_giornate
...
def _fair(prob: float | None) -> float | None:
    ...
    from goalmodel.models.baseline import fair_odds
...
def rounds(season: str, src: Sources | None = None) -> list[dict]:
    ...
    for number in sorted(numbers):
        ...
        out.append({
            ...
            "status": ROUND_STATUS.get(_opt_str(state.get("stato"))),
...
def _significance(resolved: list[dict]) -> dict:
    ...
    from goalmodel.evaluation.evaluate import cluster_bootstrap
...
def track_record(season: str, src: Sources | None = None) -> dict | None:
    ...
    if len(resolved) >= CALIBRATION_MIN_MATCHES:
        from goalmodel.evaluation.evaluate import _onehot, calibration_table, outcome_index
...
def freshness(src: Sources | None = None) -> dict:
    ...
    return {
        ...
        "matches_logged": len(snap.predictions),
        "seasons": seasons(src),
    }
